...
But our approach does not seem to be very good, since we can only really do something about any of the processes failing when they're idle. When a job is being processed, we cannot reliably do something about it, in which case we (so far) simply fail the entire job. There are certainly more robust alternatives to this, but we never got that far.
