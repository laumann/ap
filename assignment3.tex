...
\item[Reverse index]
  ...
  \begin{itemize}
    \item Mapper: Unpacks a given \texttt{Track}, unzips the word bags to get a list of indices and passes \texttt{\{MXMID, Idxs\}} to the reducer (just as in \texttt{grep}).
    \item Reducer: Keeps track of the dictionary being built up, and given \texttt{\{MXMID, Idxs\}} folds up the list of indices, for each index looking up the word it refers to and updating the dictionary in the following way:
      \[
